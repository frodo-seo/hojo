...
- 통신비·월세·구독 → 정기 지출
- 급여명세서·정기 이체 → 정기 수입
- **한 이미지에 지출·수입·정기 지출·정기 수입이 섞여 있어도 라인별로 구분해서 저장** (카드 알림 리스트처럼 여러 건이 한 화면에 있는 경우)
- **라인별 통화 감지** — 한국 카드 해외 결제처럼 원화·외화가 섞여 있으면 외화(USD·EUR·JPY·GBP) 원본 금액으로 인식 후 자동 환산
...
- **스크린샷 단일 진입점** — 영수증·결제·급여명세·청구서·증권/거래소 화면을 자동 처리
- **혼합 장부 인식** — 한 이미지에 지출·수입·정기 지출·정기 수입이 섞여 있어도 라인별로 타입을 판별
- **다중 통화 지원** — KRW / USD / EUR / JPY / GBP. 외화는 실시간 환율로 기준통화 자동 환산. 카드 알림 리스트처럼 라인마다 통화가 달라도 개별 처리
