...
- **스크린샷 우선 입력** — 이미지만 올리면 OCR → 분류 → 파싱까지 자동. 파싱 결과에 빠진 값이 있으면 확인 단계에서 직접 보완할 수 있습니다.
...
- **결제 알림 자동 파싱** — 토스·카카오페이·네이버페이·삼성페이·카드·은행 등 17개 앱 알림을 수신해 Haiku가 금액·가맹점·카테고리 추출. Home 감지 카드에서 확인·저장
- **자산 OCR 다중 종목** — 증권 앱 스크린샷 한 장에 여러 종목이 있어도 한 번에 인식
...
- 저녁 9시 기록 리마인더
- CSV 내보내기
- 같은 결제가 여러 알림으로 오는 경우 자동 중복 제거 (pkg+금액+가맹점, 10초 윈도)

### 알려진 한계

- 일부 카드사 앱(예: 현대카드)은 알림을 `RemoteViews` 커스텀 뷰로 보내기 때문에 알림 리스너가 본문을 읽을 수 없습니다. 해당 앱은 현재 미지원이며, 영수증 스크린샷으로 입력할 수 있습니다.
